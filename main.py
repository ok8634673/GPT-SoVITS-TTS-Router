...
async def tts_router(request: TTSRequest):
    ...
    # 拆分句子
    split_texts = split_sentence(request.text, settings["sentence_split_length"])
    if not split_texts:
    ...
    for i, text_part in enumerate(split_texts):
        server = enabled_servers[i % len(enabled_servers)]
...
class TTSRouterGUI:
    ...
    def create_settings_tab(self):
        ...
        split_length_label.grid(row=0, column=0, padx=10, pady=10, sticky=tk.W)
        self.split_length_var = tk.IntVar()
        split_length_entry = ttk.Entry(settings_form_frame, textvariable=self.split_length_var, width=20)
        split_length_entry.grid(row=0, column=1, padx=10, pady=10, sticky=tk.W)
        
        # 超时时间
        timeout_label = ttk.Label(settings_form_frame, text="超时时间 (秒):")
        timeout_label.grid(row=1, column=0, padx=10, pady=10, sticky=tk.W)
        self.timeout_var = tk.IntVar()
        timeout_entry = ttk.Entry(settings_form_frame, textvariable=self.timeout_var, width=20)
        timeout_entry.grid(row=1, column=1, padx=10, pady=10, sticky=tk.W)
        
        # 重试次数
        retry_label = ttk.Label(settings_form_frame, text="重试次数:")
        retry_label.grid(row=2, column=0, padx=10, pady=10, sticky=tk.W)
        self.retry_var = tk.IntVar()
        retry_entry = ttk.Entry(settings_form_frame, textvariable=self.retry_var, width=20)
        retry_entry.grid(row=2, column=1, padx=10, pady=10, sticky=tk.W)
